...
- tarots は players と同じ長さで、各席の切り札の番号（文字列）か null（その席には配らない）。
  長さの違い、知らない番号、null 以外の重複は 400。
